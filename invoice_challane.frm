...
Begin VB.Form INVOICE 
   ...
   ControlBox      =   0   'False
   Icon            =   "invoice_challane.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form2"
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.ComboBox Bookcode 
      Height          =   2325
      ItemData        =   "invoice_challane.frx":000C
      Left            =   2655
      List            =   "invoice_challane.frx":000E
      Sorted          =   -1  'True
      Style           =   1  'Simple Combo
      TabIndex        =   43
      Top             =   2550
...
rs1.Open "invoicea", CON, adOpenKeyset, adLockReadOnly, adCmdTable
...
If Not rs1.EOF Then
    ...
    If kkk.State = 1 Then
        kkk.Close
    End If
    kkk.Open "select * from sledger where subledger='" + Trim(rs1!subledger) + "'", CON, adOpenKeyset, adLockReadOnly, adCmdText
    If Not kkk.EOF Then
    ...
    kk.Open "select * from invoiceb where invoiceno=" + Trim(rs1!INVOICENO) + " order by discount,sno ", CON, adOpenKeyset, adLockReadOnly, adCmdText
    If Not kk.BOF Then
        kk.MoveFirst
        ...
        Do While Not kk.EOF
            ...
            Do While kk!discount = cdiscount
                tdata.Open "Select bookname from books where bookcode='" + Trim(kk!Bookcode) + "'", CON, adOpenKeyset, adLockReadOnly, adCmdText
...
printagain2:
                ...
                Line = Line + 1
                tdata.Open "select sum(amount)as sumamt, sum(amount-netamount) as sumdis from invoiceb where invoiceno=" + Trim(rs1!INVOICENO) + " and discount=" + Trim(Str(cdiscount)) + " group by discount", CON, adOpenKeyset, adLockReadOnly, adCmdText
       ...
       If kk.State = 1 Then
             kk.Close
       End If
       kk.Open "Select * from invoicec where invoiceno=" + Trim(Me.I_NO.Text), CON, adOpenKeyset, adLockReadOnly, adCmdText
       If Not kk.BOF Then
            Do While Not kk.EOF
                If kk!amount > 0 Then
        ...
        kk.Close
        Dim Va As Variant
        kk.Open "Select * from invoicea where invoiceno=" + Trim(Me.I_NO.Text), CON, adOpenKeyset, adLockReadOnly, adCmdText
        If Not kk.BOF Then
             If kk!txt1a <> 0 Then
...
mna.Caption = Format(Round((totalamount + otheramount - otherdiscount), 2), "0.00")
End Sub
Sub invoiceabandon()
        Me.Commandadd.Enabled = True
        Me.Commandedit.Enabled = True
...
End If
If edit = False Then
  '  kk.Open "select max(invoiceno) from INVOICEA", con, adOpenStatic, adLockReadOnly, adCmdText
   ' If kk(0) > 0 Then
   '     kk.MoveLast
    ...
    Select Case Grid1.col
            Case 1
                ...
                If rs.State = 1 Then
                    rs.Close
                End If
                rs.Open "books", CON, adOpenKeyset, adLockReadOnly, adCmdTable
                row = Grid1.row
                col = Grid1.col
...
Else
  ...
  rs1.Open "select *  from AgentMaster where AgentName='" & cmbAgentName.Text & "' order by agentname", CON, adOpenKeyset, adLockReadOnly, adCmdText
...
'cr1.GroupSelectionFormula = ""
       cr1.SelectionFormula = ""
        cr1.SelectionFormula = "{invoicea.invoiceno} = " & INVOICE.I_NO.Text & " "
        cr1.ReportFileName = App.Path & "\reports\invoicenew.rpt"
...
inv1.Open "SELECT MAX(INVOICENO) FROM INVOICEA", CON, adOpenKeyset, adLockReadOnly, adCmdText
    ...
    If edit = False Then
       If CON.Execute("Select max(invoiceno) from invoicea")(0) >= Val(Trim(Me.I_NO.Text)) Then
          ...
          Me.I_NO.Text = CON.Execute("Select max(invoiceno) from invoicea")(0) + 1
           inv = CON.Execute("Select max(invoiceno) from invoicea")(0)
...
If Len(myvalue) > 0 And Val(myvalue) > 0 Then
    ...
    If rs.State = 1 Then
        rs.Close
    End If
    rs.Open "select * from books order by BOOKCODE", CON, adOpenKeyset, adLockReadOnly, adCmdText
    row = Grid1.row
    col = Grid1.col
    If Not rs.BOF Then
        rs.MoveFirst
...
Private Sub Commandedit_Click()
    ...
    CON.Execute ("delete * from invoicectmp")
    DoEvents
...
'cr1.GroupSelectionFormula = ""
       cr1.SelectionFormula = ""
        cr1.SelectionFormula = "{invoicea.invoiceno} = " & INVOICE.I_NO.Text & " "
        cr1.ReportFileName = App.Path & "\reports\invoice.rpt"
        cr1.DataFiles(0) = App.Path & "\" + Trim(main.directory) + "\data.mdb"
        cr1.DataFiles(1) = App.Path & "\" + Trim(main.directory) + "\data.mdb"
        cr1.DataFiles(2) = App.Path & "\" + Trim(main.directory) + "\data.mdb"
        cr1.DataFiles(3) = App.Path & "\" + Trim(main.directory) + "\data.mdb"
...
Private Sub Commandsave_Click()
    ...
    If Trim(I_NO.Text) <> "" And Trim(i_dt.Text) <> "" And Trim(customercode.Text) <> "" Then
            ...
            If edit Then
                ...
                CON.Execute ("delete * from invoiceb where INVOICENO = " + Trim(I_NO.Text))
                CON.Execute ("delete * from invoicec where INVOICENO = " + Trim(I_NO.Text))
                CON.Execute "select * from INVOICEctmp"
            End If
            ...
            End If
            LAMOUNT = 0
            rs.Open "invoicea", CON, adOpenKeyset, adLockOptimistic, adCmdTable
            If Not edit Then
again:
           If CON.Execute("Select max(invoiceno) from invoicea")(0) >= Val(Trim(Me.I_NO.Text)) Then
           ...
           End If
            ...
            trs.Open " SELECT DISTCODE FROM SLEDGER  WHERE SUBLEDGER='" & Me.TextBox.Text & "'", CON, adOpenStatic, adLockOptimistic, adCmdText
            If Not trs.BOF Then
...
err1:
           If Not edit Then
            ...
            rs.Open "invoiceb", CON, adOpenKeyset, adLockOptimistic, adCmdTable
            'rs!invoicedate = Me.i_dt.Text
            ...
            rs.Open "invoicec", CON, adOpenKeyset, adLockOptimistic, adCmdTable
            '/******
                'Dim I, x As Integer
                ...
                For I = 1 To OTHERSALES.mrow
                    ...
                    If Trim(OTHERSALES.Grid1.Text) <> "" Then
                        ...
                        End If
                        If edit Then
                        temprs.Open "select * from INVOICEctmp", CON, adOpenKeyset, adLockReadOnly, adCmdText
                        ...
                        temprs.Open "select * from INVOICEEND", CON, adOpenKeyset, adLockReadOnly, adCmdText
                        If OTHERSALES.Grid1.Text <> "" Then
                                temprs.Find "TEXT='" + Trim(OTHERSALES.Grid1.Text) + "'"
                                rs!Genledger = Trim(temprs!Genledger)
                                rs!subledger = Trim(temprs!subledger)
...
Private Sub customercode_LostFocus()
    ...
    rs.Open "select * from sledger where gledger='SUNDRY DEBTORS' and subledger='" + Trim(customercode.Text) + "'", CON, adOpenKeyset, adLockReadOnly, adCmdText
...
Private Sub Form_Load()
    ...
    Grid1.ColWidth(10) = 850
    
    Bookname.Height = 2325
    Me.CommandPrint.Enabled = True
    Me.cmdHeader.Enabled = True
    Me.Commandprintnh.Enabled = True
    rs.Open "select * from books where ucase(trim(GROUPCODE))=ucase(trim('Yes'))", CON, adOpenKeyset, adLockReadOnly, adCmdText
    ...
    rs.Open "select * from sledger where gledger='" + Trim(Genledger.Text) + "'", CON, adOpenKeyset, adLockReadOnly, adCmdText
    If Not rs.BOF Then
        Do While Not rs.EOF
    ...
    rs.Open "select  Agentname from AgentMaster order by agentname", CON, adOpenKeyset, adLockReadOnly, adCmdText
    cmbAgentName.Clear
    If Not rs.EOF Then
       Do While Not rs.EOF
    ...
    Else
       ...
       kk.Open "SELECT MAX(INVOICENO) FROM INVOICEA", CON, adOpenKeyset, adLockReadOnly, adCmdText
       If kk(0) <> "" Then
           Me.I_NO.Text = Trim(Str(kk(0) + 1))
       Else
          Me.I_NO.Text = "1"
...
Set rs = New ADODB.Recordset
    ...
    Else
        If rs.State = 1 Then
           rs.Close
        End If
        ''rs.Open "INVOICEA", con, adOpenKeyset, adLockReadOnly, adCmdTable
...
'Commandedit.Enabled = True
       ...
       ' Unload OTHERSALES
        CON.Execute "select * from INVOICEctmp"
       ' OTHERSALES.Form_Load
       ' 'rs.Open "INVOICEB", con, adOpenKeyset, adLockReadOnly, adCmdTable
       rs.Open "Select * from INVOICEB where INVOICENO =" + Trim(I_NO.Text) + "", CON, adOpenStatic, adLockReadOnly
       '' rs.Find "INVOICENO='" + Trim(I_NO.Text) + "'"
       Grid1.TopRow = 2
        If Not rs.EOF Then
           ...
           Set rs = New ADODB.Recordset
            ...
            Select Case Grid1.col
                ...
                Grid1.Text = tempmeb.Text
                    ...
                    rs.Open "select * from books where GROUPCODE='Yes'", CON
                    If Not rs.BOF Then
    ...
    If Not rs1.EOF Then
        Print #1, " To,"; Tab(T1 - 3); rs1!subledger; Tab(T5); "Invoice No. : "; Trim(rs1!INVOICENO); Tab(T8 + 5); "Dt. "; Tab(T8 + 12); rs1!invoicedate 'Chr(27) + Chr(15);
            ...
            If kk.State = 1 Then
                kk.Close
            End If
            kk.Open "select * from invoiceb where invoiceno=" + Trim(rs1!INVOICENO) + " order by discount,printorder", CON, adOpenKeyset, adLockReadOnly, adCmdText
            If Not kk.BOF Then
                kk.MoveFirst
                Dim cdiscount As Double
                Dim sno As Integer
...
printagain2:
                        ...
                        tdata.Open "select sum(amount) from invoiceb where invoiceno=" + Trim(rs1!INVOICENO) + " and discount=" + Trim(Str(cdiscount)) + " group by discount", CON, adOpenKeyset, adLockReadOnly, adCmdText
                        If Not tdata.BOF Then

...
rs1.Open "select * from ChallanA where " & stridnyear, CON, adOpenKeyset, adLockReadOnly, adCmdText
...
If Not rs1.EOF Then
    ...
    If kkk.State = 1 Then
        kkk.Close
    End If
    kkk.Open "select * from sledger where subledger='" + Trim(rs1!subledger) + "' and " & stridnyear, CON, adOpenKeyset, adLockReadOnly, adCmdText
    If Not kkk.EOF Then
    ...
    kk.Open "select * from Challanb where invoiceno=" + Trim(rs1!INVOICENO) + " and  " & stridnyear & "   order by discount,sno ", CON, adOpenKeyset, adLockReadOnly, adCmdText
    If Not kk.BOF Then
        kk.MoveFirst
...
printagain2:
                ...
                Line = Line + 1
                tdata.Open "select sum(amount)as sumamt, sum(amount-netamount) as sumdis from Challanb where  " & stridnyear & "   and invoiceno=" + Trim(rs1!INVOICENO) + " and discount=" + Trim(Str(cdiscount)) + " group by discount", CON, adOpenKeyset, adLockReadOnly, adCmdText
       ...
       If kk.State = 1 Then
             kk.Close
       End If
       kk.Open "Select * from Challanc where  " & stridnyear & "   and invoiceno=" + Trim(Me.I_NO.Text), CON, adOpenKeyset, adLockReadOnly, adCmdText
       If Not kk.BOF Then
            Do While Not kk.EOF
                If kk!amount > 0 Then
        ...
        kk.Close
        Dim Va As Variant
        kk.Open "Select * from ChallanA where  " & stridnyear & "   and invoiceno=" + Trim(Me.I_NO.Text), CON, adOpenKeyset, adLockReadOnly, adCmdText
        If Not kk.BOF Then
             If kk!txt1a <> 0 Then
...
'cr1.GroupSelectionFormula = ""
       ...
       cr1.ReportFileName = strrptpath & "\reports\challanWithHead.rpt"
...
inv1.Open "SELECT MAX(INVOICENO) FROM ChallanA where " & stridnyear, CON, adOpenKeyset, adLockReadOnly, adCmdText
...
Private Sub Commandadd_Click()
    ...
    If edit = False Then
       If CON.Execute("Select max(invoiceno) from ChallanA where " & stridnyear)(0) >= Val(Trim(Me.I_NO.Text)) Then
          
          Me.I_NO.Text = CON.Execute("Select max(invoiceno) from ChallanA where " & stridnyear)(0) + 1
           inv = CON.Execute("Select max(invoiceno) from ChallanA where " & stridnyear)(0)
...
If MsgBox("Are You Sure, Delete it ?", vbYesNo) = vbNo Then
        Exit Sub
Else
                CON.Execute ("DELETE from ChallanA where  " & stridnyear & "   and INVOICENO = " + Trim(I_NO.Text))
                CON.Execute ("DELETE from Challanb where  " & stridnyear & "   and INVOICENO = " + Trim(I_NO.Text))
                CON.Execute ("DELETE from Challanc where  " & stridnyear & "   and INVOICENO = " + Trim(I_NO.Text))
                invoiceabandon
End If
End Sub
...
Private Sub Commandedit_Click()
    ...
    CON.Execute ("DELETE from Challanctmp where " & stridnyear)
    DoEvents
    CON.Execute ("insert into Challanctmp  select * from Challanc where  " & stridnyear & "   and INVOICENO = " + Trim(I_NO.Text))
    DoEvents
...
'cr1.GroupSelectionFormula = ""
        ...
        cr1.SelectionFormula = ""
        cr1.SelectionFormula = "{invoicea.invoiceno} = " & Me.I_NO.Text & " "
        cr1.ReportFileName = strrptpath & "\reports\challan.rpt"
        MaxBookNo (Me.I_NO.Text)
        cr1.Formulas(0) = "bno='" & bookNo & "'"
...
Private Sub Commandsave_Click()
    ...
    If Trim(I_NO.Text) <> "" And Trim(i_dt.Text) <> "" And Trim(customercode.Text) <> "" Then
            ...
            If edit Then
                ...
                CON.Execute ("DELETE from Challanb where  " & stridnyear & "   and INVOICENO = " + Trim(I_NO.Text))
                CON.Execute ("DELETE from Challanc where  " & stridnyear & "   and INVOICENO = " + Trim(I_NO.Text))
                CON.Execute "select * from Challanctmp where " & stridnyear
            End If
            ...
            End If
            LAMOUNT = 0
            rs.Open "select * from ChallanA where " & stridnyear, CON, adOpenKeyset, adLockOptimistic, adCmdText
            If Not edit Then
again:
           If CON.Execute("Select max(invoiceno) from ChallanA where " & stridnyear)(0) >= Val(Trim(Me.I_NO.Text)) Then
           ...
           End If
            ...
            rs!txt1 = Trim(ChallanOther.T1TEXT.Text)
            rs!txt1a = Val(Trim(ChallanOther.T1.Text))
            rs!txt2 = Trim(ChallanOther.T2TEXT.Text)
            rs!txt2a = Val(Trim(ChallanOther.T2.Text))
            rs!baa = Val(Trim(ChallanOther.T3TEXT.Text))
            rs!baa = Val(Trim(labelbybank.Caption))
            If addmode = True Then
            
            If Val(Trim(ChallanOther.T3TEXT.Text)) <> 0 Then
            ...
            End If
                ...
                For I = 1 To ChallanOther.Grid1.Rows - 1
                        If I = 1 Then
                        rs!aexp1 = ChallanOther.Grid1.TextMatrix(1, 0)
                        rs!aexp1rate = Val(ChallanOther.Grid1.TextMatrix(1, 1))
                        rs!aexp1am = Val(ChallanOther.Grid1.TextMatrix(1, 2))
                        End If
                        
                        If I = 2 Then
                        rs!aexp2 = ChallanOther.Grid1.TextMatrix(2, 0)
                        rs!aexp2rate = Val(ChallanOther.Grid1.TextMatrix(2, 1))
                        rs!aexp2am = Val(ChallanOther.Grid1.TextMatrix(2, 2))
                        End If
                        
                        If I = 3 Then
                        rs!aexp3 = ChallanOther.Grid1.TextMatrix(3, 0)
                        rs!aexp3rate = Val(ChallanOther.Grid1.TextMatrix(3, 1))
                        rs!aexp3am = Val(ChallanOther.Grid1.TextMatrix(3, 2))
                        End If
                        
                        If I = 4 Then
                        rs!lexp1 = ChallanOther.Grid1.TextMatrix(4, 0)
                        rs!lexp1rate = Val(Val(ChallanOther.Grid1.TextMatrix(4, 1)))
                        rs!lexp1am = Val(ChallanOther.Grid1.TextMatrix(4, 2))
                        End If
                        
                        If I = 5 Then
                        rs!lexp2 = ChallanOther.Grid1.TextMatrix(5, 0)
                        rs!lexp2rate = Val(ChallanOther.Grid1.TextMatrix(5, 1))
                        rs!lexp2am = Val(ChallanOther.Grid1.TextMatrix(5, 2))
                        End If
                        
                        If I = 6 Then
                        rs!lexp3 = ChallanOther.Grid1.TextMatrix(6, 0)
                        rs!lexp3rate = Val(ChallanOther.Grid1.TextMatrix(6, 1))
                        rs!lexp3am = Val(ChallanOther.Grid1.TextMatrix(6, 2))
                        End If
                        
                        'ChallanOther.Grid1.row = ChallanOther.Grid1.row + 1
                Next
...
err1:
           If Not edit Then
            ...
            rs.Open "select * from Challanb where " & stridnyear, CON, adOpenKeyset, adLockOptimistic, adCmdText
            'rs!invoicedate = Me.i_dt.Text
            ...
            rs.Open "select * from Challanc where " & stridnyear, CON, adOpenKeyset, adLockOptimistic, adCmdText
            '/******
                'Dim I, x As Integer
                ...
                For I = 1 To ChallanOther.mrow
                    ...
                    If Trim(ChallanOther.Grid1.Text) <> "" Then
                        rs.AddNew
                        rs!INVOICENO = Val(Me.I_NO.Text)
                        rs!invoicedate = Me.i_dt.Text
                        rs!gamount = (Me.totalamount - Me.totaldiscount)
                        rs!Text = Trim(ChallanOther.Grid1.Text)
                        If temprs.State = 1 Then
                            temprs.Close
                        End If
                        If edit Then
                        temprs.Open "select * from Challanctmp where " & stridnyear, CON, adOpenKeyset, adLockReadOnly, adCmdText
                        If ChallanOther.Grid1.Text <> "" Then
                                temprs.Find "TEXT='" + Trim(ChallanOther.Grid1.Text) + "'"
                        ...
                        temprs.Open "select * from INVOICEEND where " & stridnyear, CON, adOpenKeyset, adLockReadOnly, adCmdText
                        If ChallanOther.Grid1.Text <> "" Then
                                temprs.Find "TEXT='" + Trim(ChallanOther.Grid1.Text) + "'"
                                rs!Genledger = Trim(temprs!Genledger)
                                rs!subledger = Trim(temprs!subledger)
                        ...
                        temprs.Close
                        End If
                        ChallanOther.Grid1.col = 1
                        rs!rate = Val(Trim(ChallanOther.Grid1.Text))
                        If Val(Trim(ChallanOther.Grid1.Text)) > 0 Then
                            rs!amount = (Me.totalamount - Me.totaldiscount) * (Val(Trim(ChallanOther.Grid1.Text)) / 100)
                        Else
                        ChallanOther.Grid1.col = 2
                            rs!amount = Val(Trim(ChallanOther.Grid1.Text))
                        End If
            ...
            rs!fyear = main.session: rs!setupid = main.setupid
                ...
                rs!updatedon = Now
                End If
                rs!In = CON.Execute("Select max(invoiceno) from ChallanA")(0)
                rs.Update
                rs.Close
...
Private Sub Commandsearch_Click()
    ...
    CON.Execute "select * from Challanctmp where " & stridnyear
    Me.Enabled = False
    Call searchscreen.tempr(112, "challannew")
End Sub
...
Private Sub Form_Load()
    ...
    Else
       ...
       kk.Open "SELECT MAX(INVOICENO) FROM ChallanA where   " & stridnyear & "   ", CON, adOpenKeyset, adLockReadOnly, adCmdText
       If kk(0) <> "" Then
           Me.I_NO.Text = Trim(Str(kk(0) + 1))
       Else
          Me.I_NO.Text = "1"
...
Set rs = New ADODB.Recordset
    If Trim(I_NO.Text) = "" Then
        MsgBox "Challan cannot be null"
        I_NO.SetFocus
    Else
        If rs.State = 1 Then
           rs.Close
        End If
        ''rs.Open "ChallanA", con, adOpenKeyset, adLockReadOnly, adcmdtext
        rs.Open "Select * from  ChallanA where  " & stridnyear & "   and INVOICENO = " + Trim(I_NO.Text) + "", CON, adOpenStatic, adLockReadOnly
        ''rs.Find "INVOICENO='" + Trim(I_NO.Text) + "'"
        
        If rs.EOF Then
            If addoredit = False Then
                MsgBox "Challan not found"
                Exit Sub
            End If
            Exit Sub
        End If
        If addoredit Then
            MsgBox "Challan already exist..."
            I_NO.SetFocus
            HIT
            Exit Sub
        End If
...
'Commandedit.Enabled = True
       ...
       rs.Open "Select * from Challanb where   " & stridnyear & "    and INVOICENO =" + Trim(I_NO.Text) + "", CON, adOpenStatic, adLockReadOnly
       '' rs.Find "INVOICENO='" + Trim(I_NO.Text) + "'"
       Grid1.TopRow = 2
        If Not rs.EOF Then
            ...
            Do While Not rs.EOF
               If Trim(rs!INVOICENO) = Trim(I_NO.Text) Then
                ...
                If kk.State = 1 Then
                    kk.Close
                End If
                kk.Open "select * from books where bookcode='" + Trim(rs!Bookcode) + "' and  " & stridnyear & " ", CON, adOpenStatic, adLockReadOnly, adCmdText
    ...
    If Not rs1.EOF Then
        Print #1, " To,"; Tab(T1 - 3); rs1!subledger; Tab(T5); "Invoice No. : "; Trim(rs1!INVOICENO); Tab(T8 + 5); "Dt. "; Tab(T8 + 12); rs1!invoicedate 'Chr(27) + Chr(15);
            ...
            If kk.State = 1 Then
                kk.Close
            End If
            kk.Open "select * from Challanb where  " & stridnyear & "   and invoiceno=" + Trim(rs1!INVOICENO) + " order by discount,printorder", CON, adOpenKeyset, adLockReadOnly, adCmdText
            If Not kk.BOF Then
                kk.MoveFirst
                Dim cdiscount As Double
                Dim sno As Integer
...
printagain2:
                        ...
                        tdata.Open "select sum(amount) from Challanb where  " & stridnyear & "   and invoiceno=" + Trim(rs1!INVOICENO) + " and discount=" + Trim(Str(cdiscount)) + " group by discount", CON, adOpenKeyset, adLockReadOnly, adCmdText
                        If Not tdata.BOF Then

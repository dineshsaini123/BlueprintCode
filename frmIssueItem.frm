...
Begin VB.Form frmIssueItem 
   ...
   Begin VB.ComboBox Bookcode 
      Height          =   2325
      ItemData        =   "frmIssueItem.frx":0000
      Left            =   2820
      List            =   "frmIssueItem.frx":0002
      Sorted          =   -1  'True
      Style           =   1  'Simple Combo
      TabIndex        =   10
      Top             =   2040
   ...
   End
End
Attribute VB_Name = "frmIssueItem"
...
rs1.Open "IssueA", CON, adOpenKeyset, adLockReadOnly, adCmdText
...
If Not rs1.EOF Then
    ...
    kk.Open "select * from Issueb where invoiceno=" + Trim(rs1!INVOICENO) + " order by discount,sno ", CON, adOpenKeyset, adLockReadOnly, adCmdText
    If Not kk.BOF Then
        kk.MoveFirst
...
printagain2:
                ...
                Line = Line + 1
                tdata.Open "select sum(amount)as sumamt, sum(amount-netamount) as sumdis from Issueb where invoiceno=" + Trim(rs1!INVOICENO) + " and discount=" + Trim(Str(cdiscount)) + " group by discount", CON, adOpenKeyset, adLockReadOnly, adCmdText
       ...
       If kk.State = 1 Then
             kk.Close
       End If
       kk.Open "Select * from IssueC where invoiceno=" + Trim(Me.I_NO.Text), CON, adOpenKeyset, adLockReadOnly, adCmdText
       If Not kk.BOF Then
            Do While Not kk.EOF
                If kk!amount > 0 Then
        ...
        kk.Close
        Dim Va As Variant
        kk.Open "Select * from IssueA where invoiceno=" + Trim(Me.I_NO.Text), CON, adOpenKeyset, adLockReadOnly, adCmdText
        If Not kk.BOF Then
             If kk!txt1a <> 0 Then
...
End If
If edit = False Then
  '  kk.Open "select max(invoiceno) from IssueA", con, adOpenStatic, adLockReadOnly, adCmdText
   ' If kk(0) > 0 Then
   '     kk.MoveLast
...
inv1.Open "SELECT MAX(INVOICENO) FROM IssueA", CON, adOpenKeyset, adLockReadOnly, adCmdText
...
Private Sub Commandadd_Click()
    ...
    If edit = False Then
       If CON.Execute("Select max(invoiceno) from IssueA")(0) >= Val(Trim(Me.I_NO.Text)) Then
          
          Me.I_NO.Text = CON.Execute("Select max(invoiceno) from IssueA")(0) + 1
           inv = CON.Execute("Select max(invoiceno) from IssueA")(0)
...
If MsgBox("Are You Sure, Delete it ?", vbYesNo) = vbNo Then
        Exit Sub
Else
                CON.Execute ("DELETE from IssueA where INVOICENO = " + Trim(I_NO.Text))
                CON.Execute ("DELETE from Issueb where INVOICENO = " + Trim(I_NO.Text))
                CON.Execute ("DELETE from IssueC where INVOICENO = " + Trim(I_NO.Text))
                invoiceabandon
End If
End Sub
...
Private Sub Commandedit_Click()
    ...
    CON.Execute ("DELETE from invoicectmp where " & stridnyear)
    DoEvents
    CON.Execute ("insert into invoicectmp  select * from IssueC where INVOICENO = " + Trim(I_NO.Text))
    DoEvents
...
'''        cr1.SelectionFormula = "{IssueA.invoiceno} = " & INVOICE.I_NO.Text & " "
'''        cr1.ReportFileName = strrptpath & "\reports\invoice.rpt"
...
Private Sub Commandsave_Click()
    ...
    If Trim(I_NO.Text) <> "" And Trim(i_dt.Text) <> "" And Trim(customercode.Text) <> "" Then
            ...
            If edit Then
                ...
                CON.Execute ("DELETE from Issueb where INVOICENO = " + Trim(I_NO.Text))
                CON.Execute ("DELETE from IssueC where INVOICENO = " + Trim(I_NO.Text))
                CON.Execute "select * from invoicectmp where " & stridnyear
            End If
            ...
            End If
            LAMOUNT = 0
            rs.Open "IssueA", CON, adOpenKeyset, adLockOptimistic, adCmdText
            If Not edit Then
again:
           If CON.Execute("Select max(invoiceno) from IssueA")(0) >= Val(Trim(Me.I_NO.Text)) Then
            ...
            rs!subledger = "customer" 'Trim(Me.textbox.Text)
...
err1:
           If Not edit Then
            ...
            On Error GoTo 0
            rs.Close
            rs.Open "Issueb", CON, adOpenKeyset, adLockOptimistic, adCmdText
            ...
            rs.Open "IssueC", CON, adOpenKeyset, adLockOptimistic, adCmdText
            '/******
                'Dim I, x As Integer
                ...
                For I = 1 To OTHERSALES.mrow
                    ...
                    If Trim(OTHERSALES.Grid1.Text) <> "" Then
                        ...
                        End If
                        If edit Then
                        temprs.Open "select * from invoicectmp where " & stridnyear, CON, adOpenKeyset, adLockReadOnly, adCmdText
                ...
                If rs.BOF Then
                    rs.AddNew
                End If
                rs!In = CON.Execute("Select max(invoiceno) from IssueA")(0)
                rs.Update
                rs.Close
...
Private Sub Commandsearch_Click()
    ...
    Call searchscreen.tempr(12, "IssueA")
End Sub
...
'    End If
    ...
    Me.CommandPrint.Enabled = True
    Me.Commandprintnh.Enabled = True
    rs.Open "select * from books where " & stridnyear & "  and GROUPCODE='No'", CON, adOpenKeyset, adLockReadOnly, adCmdText
    If Not rs.BOF Then
        Do While Not rs.EOF
    ...
    Else
       ...
       kk.Open "SELECT MAX(INVOICENO) FROM IssueA", CON, adOpenKeyset, adLockReadOnly, adCmdText
       If kk(0) <> "" Then
           Me.I_NO.Text = Trim(Str(kk(0) + 1))
       Else
          Me.I_NO.Text = "1"
...
Set rs = New ADODB.Recordset
    If Trim(I_NO.Text) = "" Then
        MsgBox "Issue No cannot be null"
        I_NO.SetFocus
    Else
        If rs.State = 1 Then
           rs.Close
        End If
        ''rs.Open "IssueA", con, adOpenKeyset, adLockReadOnly, adcmdtext
        rs.Open "Select * from  IssueA where INVOICENO = " + Trim(I_NO.Text) + "", CON, adOpenStatic, adLockReadOnly
        ''rs.Find "INVOICENO='" + Trim(I_NO.Text) + "'"
        
        If rs.EOF Then
            If addoredit = False Then
                MsgBox "Issue No not found"
                Exit Sub
            End If
            Exit Sub
        End If
        If addoredit Then
            MsgBox "Issue already exist..."
            I_NO.SetFocus
            HIT
            Exit Sub
        End If
...
'Commandedit.Enabled = True
       ...
       ' Unload OTHERSALES
        CON.Execute "select * from invoicectmp where " & stridnyear
       ' OTHERSALES.Form_Load
       ' 'rs.Open "Issueb", con, adOpenKeyset, adLockReadOnly, adcmdtext
       rs.Open "Select * from Issueb where INVOICENO =" + Trim(I_NO.Text) + "", CON, adOpenStatic, adLockReadOnly
       '' rs.Find "INVOICENO='" + Trim(I_NO.Text) + "'"
       Grid1.TopRow = 2
        If Not rs.EOF Then
           ...
           Set rs = New ADODB.Recordset
            ...
            Select Case Grid1.col
                ...
                Grid1.Text = tempmeb.Text
                    ...
                    rs.Open "select * from books where  " & stridnyear & " ", CON, adOpenStatic, adLockReadOnly, adCmdText
                    If Not rs.BOF Then
    ...
    If Not rs1.EOF Then
        Print #1, " To,"; Tab(T1 - 3); rs1!subledger; Tab(T5); "Invoice No. : "; Trim(rs1!INVOICENO); Tab(T8 + 5); "Dt. "; Tab(T8 + 12); rs1!invoicedate 'Chr(27) + Chr(15);
            ...
            If kk.State = 1 Then
                kk.Close
            End If
            kk.Open "select * from Issueb where invoiceno=" + Trim(rs1!INVOICENO) + " order by discount,printorder", CON, adOpenKeyset, adLockReadOnly, adCmdText
            If Not kk.BOF Then
                kk.MoveFirst
                Dim cdiscount As Double
                Dim sno As Integer
...
printagain2:
                        ...
                        tdata.Open "select sum(amount) from Issueb where invoiceno=" + Trim(rs1!INVOICENO) + " and discount=" + Trim(Str(cdiscount)) + " group by discount", CON, adOpenKeyset, adLockReadOnly, adCmdText
                        If Not tdata.BOF Then

...
Begin VB.Form frmBookStock 
   ...
   Begin VB.Frame panel 
      ...
      Begin VB.ComboBox cboserName 
         Height          =   315
         ItemData        =   "frmBookStock.frx":0000
         Left            =   5355
         List            =   "frmBookStock.frx":0002
      ...
      Begin VB.ComboBox cboGroup 
         Height          =   315
         ItemData        =   "frmBookStock.frx":27F2
         Left            =   5355
         List            =   "frmBookStock.frx":27F4
         Style           =   2  'Dropdown List
         TabIndex        =   5
         Top             =   360
         Width           =   2445
      ...
      Begin VB.ComboBox cboBinder_Godown 
         Height          =   288
         ItemData        =   "frmBookStock.frx":27F6
         Left            =   1530
         List            =   "frmBookStock.frx":2803
      ...
      Begin VB.ComboBox cboCategory 
         Height          =   315
         ItemData        =   "frmBookStock.frx":281C
         Left            =   1530
         List            =   "frmBookStock.frx":2829
         Style           =   2  'Dropdown List
         TabIndex        =   1
         Top             =   360
         Width           =   2310
...
ElseIf cboBinder_Godown.text = "NS" Then

    If stock_Trans = 1 Then
    ...
    Else
        ...
        contr.Execute "delete from BookOpening_NS where Godown='" & cboBinder_Godown.text & "' and bookgp='" & cboGroup.text & "'"
        For I = 1 To vs.rows - 1
            If vs.TextMatrix(I, 0) <> "" Then
               If cboBinder_Godown.text <> "" Then
                contr.Execute "insert into BookOpening_NS(BOOKCODE,ItemName,Balance,Godown,bookgp) values('" & vs.TextMatrix(I, 0) & "','" & vs.TextMatrix(I, 1) & "','" & vs.TextMatrix(I, 2) & "','" & cboBinder_Godown.text & "','" & cboGroup.text & "')"
               End If
            End If
        Next
 ...
 For I = 1 To vs.rows - 1
   If vs.TextMatrix(I, 0) <> "" Then
      If (vs.Cols = 3) Then
        ...
        "'" & vs.TextMatrix(I, 2) & "','" & UserName & "','" & cboBinder_Godown & "','" & dateAson.value & "','" & 0 & "','" & 0 & "')"
      ElseIf vs.Cols > 3 Then
...
vs.Cols = 5
Set rs1 = con.Execute("exec PendingOrder_Sale_Specimen '" & cboBinder_Godown.text & "','" & cboGroup.text & "'")

For k2 = 1 To vs.rows - 1
    
    
'    If (vs.TextMatrix(k2, 0) = "CBR1") Then
'    MsgBox "ssss"
'    End If
...
lblStokType.Caption = "Stock Reflect With Order"
stock_Trans = 1
...
If cboBinder_Godown.text = "NS" Then
    ...
    Else
       ...
       con.Execute "insert into tmpNSStock(bookcode,Qty,QtySp,status,dates,status_) select a.BOOKCODE,sum(a.QUANTITY) as QUANTITY,sum(a.SpQty) as SpQty,'IN' as Issue_Rec,b.invoiceDate,'Stock IN By Challan' FROM CHALANB_ret as a inner join CHALANa_ret as b on (a.INVOICENO = b.INVOICENO) group by a.BOOKCODE,b.invoiceDate"
       con.Execute "update tmpNSStock set Qty=0 where Qty is null"
       con.Execute "update tmpNSStock set QtySp=0 where QtySp is null"
    End If
...
con.Execute "delete from tmpNSStock_ORDER"

con.Execute "insert into tmpNSStock_ORDER(bookcode,Qty,[status],dates,status_,gd) SELECT Distinct [Book_code] ,(Qty) as QUANTITY,'In' as Issue_Rec,invoicedate,'BinderRec' as status_,'" & cboBinder_Godown.text & "' FROM BinderReceiveRegister WHERE Godown ='" & cboBinder_Godown.text & "'"
con.Execute "insert into tmpNSStock_ORDER(bookcode,Qty,[status],dates,status_,gd) SELECT Distinct [Book_code] ,(Qty*-1) as QUANTITY,'Out' as Issue_Rec,invoicedate,'BinderIssue' as status_,'" & cboBinder_Godown.text & "' FROM BinderIssueRegister WHERE Godown ='" & cboBinder_Godown.text & "'"

con.Execute "insert into tmpNSStock_ORDER(bookcode,Qty,[status],dates,status_,gd) SELECT Distinct [Bookcode] ,(Qty) as QUANTITY,'In' as Issue_Rec,invoicedate,'Stock In(By Cr.NotItem-FREE' as status_,'" & cboBinder_Godown.text & "' FROM SaleReturnRegister_Free WHERE Godown ='" & cboBinder_Godown.text & "'"
con.Execute "insert into tmpNSStock_ORDER(bookcode,Qty,[status],dates,status_,gd) SELECT Distinct [Bookcode] ,(QUANTITY) as Qty,'In' as Issue_Rec,invoicedate,'Stock In(By Cr.NotItem' as status_,'" & cboBinder_Godown.text & "' FROM CreditbQry WHERE Godown ='" & cboBinder_Godown.text & "'"

con.Execute "insert into tmpNSStock_ORDER(bookcode,QtySp,[status],dates,status_,gd) SELECT Distinct [Bookcode] ,Qty,'In' as Issue_Rec,invoicedate,'Stock In(By Sp.Ret-FREE)' as status_,'" & cboBinder_Godown.text & "' FROM SpecimenReturnRegister_Free  WHERE Godown ='" & cboBinder_Godown.text & "'"
con.Execute "insert into tmpNSStock_ORDER(bookcode,QtySp,[status],dates,status_,gd) SELECT Distinct [Bookcode] ,Qty,'In' as Issue_Rec,invoicedate,'Stock In(By Sp.Ret)' as status_,'" & cboBinder_Godown.text & "' FROM SpecimenReturnRegister  WHERE Godown ='" & cboBinder_Godown.text & "'"

con.Execute "insert into tmpNSStock_ORDER(bookcode,Qty,status,dates,status_,gd)  SELECT [BOOKCODE],(sum(Qty)*-1) as Qty,'Out' as Issue_Rec,dates,'BookStock Out','" & cboBinder_Godown.text & "' FROM BookStock where (Godown_out ='" & cboBinder_Godown.text & "') group by BOOKCODE,dates"
con.Execute "insert into tmpNSStock_ORDER(bookcode,Qty,status,dates,status_,gd)  SELECT [BOOKCODE],sum(Qty) as Qty,'IN' as Issue_Rec,dates,'BookStock IN','" & cboBinder_Godown.text & "' FROM BookStock where (Godown_in ='" & cboBinder_Godown.text & "') group by BOOKCODE,dates"
con.Execute "insert into tmpNSStock_ORDER(bookcode,Qty,QtySp,status,dates,status_,gd)  SELECT [BOOKCODE],(sum(QUANTITY)*-1) as Qty,(sum(SpQty)*-1) as SpQty,'Out' as Issue_Rec,INVOICEDATE,'Stock Out By Order','" & cboBinder_Godown.text & "' FROM OrderBookList  where  Godown ='" & cboBinder_Godown.text & "' group by BOOKCODE,invoiceDate"
con.Execute "insert into tmpNSStock_ORDER(bookcode,Qty,status,dates,status_,gd)  SELECT  BOOKCODE,Balance,'Out' as Issue_Rec,'" & fromDate_setup & "','Opening Qty in (-)','" & cboBinder_Godown.text & "' FROM BookOpening where (Godown ='" & cboBinder_Godown.text & "' and Balance<0)"
con.Execute "insert into tmpNSStock_ORDER(bookcode,Qty,status,dates,status_,gd)  SELECT  BOOKCODE,Balance,'IN' as Issue_Rec,'" & fromDate_setup & "','Opening Qty in (+)','" & cboBinder_Godown.text & "' FROM BookOpening where (Godown ='" & cboBinder_Godown.text & "' and Balance>0)"
...
Else
  ...
  If (txtBookCode.text = "" Or txtBookCode.text = "All") Then
    
    If cboserName.text = "" Then
     rs_data.Open "select  tmpNSStock_ORDER.BookCode,sum(Qty),sum(QtySP),BOOKS.GROUPCODE from tmpNSStock_ORDER  tmpNSStock_ORDER INNER JOIN books ON tmpNSStock_ORDER.BookCode = BOOKS.BOOKCODE " & _
     " where BOOKS.GROUPCODE='" & cboGroup.text & "'  and convert(smalldatetime,Dates,103)<= convert(smalldatetime,'" & dateAson.value & "',103) group by tmpNSStock_ORDER.BookCode,books.groupcode order by tmpNSStock_ORDER.BookCode", con, adOpenDynamic, adLockOptimistic
    Else
         rs_data.Open "select  tmpNSStock_ORDER.BookCode,sum(Qty),sum(QtySP),BOOKS.GROUPCODE from tmpNSStock_ORDER  tmpNSStock_ORDER INNER JOIN books ON tmpNSStock_ORDER.BookCode = BOOKS.BOOKCODE " & _
     " where BOOKS.GROUPCODE='" & cboGroup.text & "' and BOOKS.sername='" & cboserName.text & "'  and convert(smalldatetime,Dates,103)<= convert(smalldatetime,'" & dateAson.value & "',103) group by tmpNSStock_ORDER.BookCode,books.groupcode order by tmpNSStock_ORDER.BookCode", con, adOpenDynamic, adLockOptimistic

    End If
    
 Else
     
     If txtBookCode.text <> "All" Then
        rs_data.Open "select  tmpNSStock_ORDER.BookCode,sum(Qty),sum(QtySP),BOOKS.GROUPCODE from tmpNSStock_ORDER  tmpNSStock_ORDER INNER JOIN books ON tmpNSStock_ORDER.BookCode = BOOKS.BOOKCODE " & _
        " where BOOKS.GROUPCODE='" & cboGroup.text & "' and BOOKS.bookcode='" & txtBookCode.text & "' and convert(smalldatetime,Dates,103)<= convert(smalldatetime,'" & dateAson.value & "',103) group by tmpNSStock_ORDER.BookCode,books.groupcode order by tmpNSStock_ORDER.BookCode", con, adOpenDynamic, adLockOptimistic
...
lblStokType.Caption = "Not Stock Reflect With Order"
...
If stock_Trans = 2 Then
    con.Execute "insert into tmpNSStock(bookcode,Qty,status,dates,status_)  SELECT distinct BOOKCODE,Balance,'Out' as Issue_Rec,'" & fromDate_setup & "','Opening Qty in (-)' FROM BookOpening_ns where (Godown ='NS' and Balance<0)"
    con.Execute "insert into tmpNSStock(bookcode,Qty,status,dates,status_)  SELECT distinct BOOKCODE,Balance,'IN' as Issue_Rec,'" & fromDate_setup & "','Opening Qty in (+)' FROM BookOpening_ns where (Godown ='NS' and Balance>0)"
End If
...
If Check1_sp.value = 0 Then
    ...
    If rs_kit.State = 1 Then rs_kit.close
    rs_kit.Open "select KITCODE from BOOKS_KIT  where BOOKCODE='" & rs_data.Fields("BOOKCODE").value & "'", con
    'If rs_kit.EOF = False Then
...
" (BookCode='" & rs_data.Fields("BOOKCODE").value & "' and BookCode='" & rs_data.Fields("BOOKCODE").value & "') and " & _
" " & str1 & " and " & stringyear & "", con, adOpenKeyset
If Not IsNull(RS(0)) Then
...
''------------Kit Qry (CashRegister)-----------------------------------------

If Check1_sp.value = 0 Then

If rs_cash.State = 1 Then rs_cash.close
rs_cash.Open "select sum(QUANTITY) from CashB_Free where convert(smalldatetime,invoiceDate,103)<= convert(smalldatetime,'" & dateAson & "',103) and " & _
" (BookCode='" & rs_data.Fields("BOOKCODE").value & "' and BookCode='" & rs_data.Fields("BOOKCODE").value & "') and " & _
" " & str1 & " and " & stringyear & "", con, adOpenKeyset
If Not IsNull(rs_cash(0)) Then
   godown_issue = godown_issue + rs_cash(0)
End If
...
If rs_issue.State = 1 Then rs_issue.close
rs_issue.Open "select sum(QUANTITY) from SPRegister_Free where convert(smalldatetime,invoicedate,103)<= convert(smalldatetime,'" & dateAson & "',103) and " & _
" BookCode='" & rs_data.Fields("BOOKCODE").value & "' and " & _
" " & str1 & " and " & stringyear & "", con, adOpenKeyset
If Not IsNull(rs_issue(0)) Then godown_issue = godown_issue + rs_issue(0)
...
" BOOKCODE='" & rs_data.Fields("BOOKCODE").value & "' and " & _
" godown='" & cboBinder_Godown.text & "' and " & stringyear & "", con, adOpenKeyset
If Not IsNull(RS(0)) Then godown_issue = godown_issue + RS(0)
...
If ddd = 1 Then
        ...
        If vs.TextMatrix(r, 1) <> "" Then
               ...
               xlSheet.Cells(1, 4).value = "Pending Ord.Qty"      '"Sold Last Year"
               xlSheet.Cells(1, 5).value = "Today Bal.Qty"
...
'                      xlSheet.Cells(r1, 4).value = soldTillDate
'                  End If
'               '----------------
'               soldTillDate = 0
'
...
'
'               If Not IsNull(RS(0)) Then soldTillDate = RS(0)
'                xlSheet.Cells(r1, 5).value = soldTillDate
...
If RS.State = 1 Then RS.close
RS.Open "select GROUPCODE from BOOKS where " & stringyear & " group by GROUPCODE order by GROUPCODE", con
While RS.EOF = False

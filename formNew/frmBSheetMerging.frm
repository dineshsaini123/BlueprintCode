...
Begin VB.Form frmBSheet 
   Caption         =   "Balance Sheet (Merging Report)"
   ClientHeight    =   10200
   ClientLeft      =   48
   ClientTop       =   396
   ClientWidth     =   16476
   Icon            =   "frmBSheetMerging.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   10200
   ...
   Begin VB.ComboBox cbogl 
      ...
      EndProperty
      Height          =   288
      ItemData        =   "frmBSheetMerging.frx":000C
      Left            =   144
      List            =   "frmBSheetMerging.frx":0016
      TabIndex        =   26
      Top             =   432
      Width           =   2688
   End
...
Set rs1 = con.Execute("exec spBalanceSheet_Mearging '" & Format(date1.text, "MM/dd/yyyy") & "','" & Format(date2.text, "MM/dd/yyyy") & "','" & cbogl.text & "'")
 K = 1
...
While rs1.EOF = False
 ...
 vs.rows = vs.rows + 1
 vs.TextMatrix(K, 0) = K
 vs.TextMatrix(K, 1) = rs1(0)
 vs.TextMatrix(K, 2) = Round(rs1(2), 0)
 K = K + 1
...
Wend
    ...
    vs.FormatString = "SrNo|Account Name|Total Amount|More Then"
...
Sub showdatainGrid()
    ...
    vs.TextMatrix(K, 4) = dr
    vs.TextMatrix(K, 7) = dr

...
aa = 0
con.BeginTrans
con.Execute "delete from CashRegister_conven"
con.Execute "insert into  CashRegister_conven(SNO,[Date],Particulars,BDL,WT,Freight,RR,GR,GR_DT,Freight_Paid,CMNo) SELECT INVOICENO, INVOICEDATE,AgentName,STATION,BUNDLES,transportname,MARKA,BILTYNO,BILTYDATE,FREIGHT,INVOICENO FROM INVOICEA_sp where " & stringyear & ""
con.CommitTrans

ss = ""
K = 1
If RS.State = 1 Then RS.close
RS.Open "select station,invoiceno from INVOICEA_sp order by invoiceno", con, adOpenKeyset, adLockReadOnly
While RS.EOF = False
...
con.Execute "update CashRegister_conven set bdl='" & ss & "',sno=" & K & "  where CMNo=" & RS(1) & ""
...
'CON.Execute "update CashRegister_conven set bdl='' where len(bdl)=2"
...
rs1.Open "select invoiceno from INVOICEA_sp where INVOICENO not in(SELECT sno FROM CashRegister_conven)", con
While rs1.EOF = False

con.Execute "insert into  CashRegister_conven(SNO,[Date],Particulars,BDL,WT,Freight,RR,GR,GR_DT,Freight_Paid,CMNo) SELECT INVOICENO, INVOICEDATE,AgentName,STATION,BUNDLES,transportname,MARKA,BILTYNO,BILTYDATE,FREIGHT,INVOICENO FROM INVOICEA_sp where invoiceno=" & rs1(0) & " and " & stringyear & ""

ss = ""
K = 1
If RS.State = 1 Then RS.close
RS.Open "select station,invoiceno from INVOICEA_sp where invoiceno=" & rs1(0) & "", con, adOpenKeyset, adLockReadOnly
...
con.Execute "update CashRegister_conven set bdl='" & ss & "',sno=" & rs1(0) & "  where CMNo=" & RS(1) & ""
...
'con.Execute "insert into  CashRegister_conven  IN '" & S1 & "' select * from CashRegister_conven"
DoEvents
cr1.ReportFileName = App.Path & "\2003-04\CashRegister.rpt"
...
cr1.ReplaceSelectionFormula ("totext({BILTYRETURNREGISTER.Recd_dt}) = '' and {CashRegister_conven.RR} <> ''")
Else
If Optiongrno = True Then
cr1.ReplaceSelectionFormula ("totext({BILTYRETURNREGISTER.Recd_dt}) = '' and {CashRegister_conven.GR} <>  0")
Else
...
Sub addData()
  ...
  adoPrimaryRS.Open "select SNO,Date,Particulars,BDL as Station,WT as Bundle,Freight as Transport,RR as PM,CMNo,GR as Bilty,GR_DT,Freight_Paid AS Fright,Remarks from CashRegister_conven where Particulars='" & customercode.Text & "' Order by SNO", con, adOpenDynamic, adLockOptimistic
...
If (UserName = "v" Or LCase(UserName) = "admin") Then
...
End If
  ...
  flg1 = True
  Set adoPrimaryRS = New Recordset
  'adoPrimaryRS.Open "select CMNo as [CHNo],Date,Particulars,BDL as Station,WT as Bundle,Freight as Transport,RR as PM,GR as Bilty,GR_DT,Freight_Paid AS Fright from CashRegister_conven where " & stringyear & " Order by CMNo", CON, adOpenDynamic, adLockOptimistic
  adoPrimaryRS.Open "select CMNo as [CHNo],Date,Particulars,BDL as Station,WT as Bundle,Freight as Transport,RR as PM,GR as Bilty,GR_DT,Freight_Paid AS Fright from CashRegister_conven Order by CMNo", con, adOpenDynamic, adLockOptimistic
  ',Remarks
  Set grdDataGrid.DataSource = adoPrimaryRS
  ...
  Dim rs1 As New ADODB.Recordset
   at = "SUNDRY DEBTORS"
    ...
    rs1.Open "select distinct(STATION) from INVOICEA_sp", con, adOpenDynamic, adLockReadOnly, adCmdText
    If Not rs1.BOF Then
        Do While Not rs1.EOF
            If rs1(0) <> "" Then
...
If h1 <> "" Then
    ...
    If flg1 = True Then
       'adoPrimaryRS.Open "select SNO,Date ,Particulars,RR,GR,GR_dt,BDL,WT,Freight,Recd_dt,Freight_Paid,CNO,RNO,Remarks from CashRegister_conven Order by  " & h1, con, adOpenDynamic, adLockOptimistic
       adoPrimaryRS.Open "select SNO,Date,Particulars,BDL as Station,WT as Bundle,Freight as Transport,RR as PM,GR as Bilty,GR_DT,Freight_Paid AS Fright,Remarks from CashRegister_conven Order by  " & h1, con, adOpenDynamic, adLockOptimistic
       flg1 = False
    Else
       'adoPrimaryRS.Open "select SNO,Date ,Particulars,RR,GR,GR_dt,BDL,WT,Freight,Recd_dt,Freight_Paid,CNO,RNO,Remarks from CashRegister_conven Order by " & h1 & " desc", con, adOpenDynamic, adLockOptimistic
       adoPrimaryRS.Open "select SNO,Date,Particulars,BDL as Station,WT as Bundle,Freight as Transport,RR as PM,GR as Bilty,GR_DT,Freight_Paid AS Fright,Remarks from CashRegister_conven Order by " & h1 & " desc", con, adOpenDynamic, adLockOptimistic
       flg1 = True
    End If
...
Dim rs1 As New ADODB.Recordset
rs1.Open "Select max(Sno)as mno from CashRegister_conven, CON, adOpenDynamic"

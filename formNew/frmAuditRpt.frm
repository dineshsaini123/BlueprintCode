...
Begin VB.Form frmAuditRpt 
   Caption         =   "Stock Audit Report..."
   ClientHeight    =   9516
   ClientLeft      =   48
   ClientTop       =   396
   ClientWidth     =   14928
   Icon            =   "frmAuditRpt.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   9516
   ScaleWidth      =   14928
   Begin VB.CheckBox Check1_withoutCr 
      Caption         =   "Print Without Cr Balancce"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.6
         Charset         =   0
...
If (session = "2022-23" Or session = "2021-22") Then
  
Else
   MsgBox "This Report can work only for 2022-23 and 2021-22 ....."
   Exit Sub
End If


If (DateValue(from_date) > DateValue(fdate.value)) Then
lastDb = "yes"
End If
...
If lastDb = "yes" Then
   ...
   con.Execute "exec balance_120 '" & fdate.value & "','" & tdate.value & "','" & UId & "'"
   
   con_LAST.Execute "exec OPbalance_120 '" & fdate.value & "'"
   
   current_Last = "last"
   
Else

   con.Execute "exec OPbalance_120 '" & fdate.value & "'"
...
con.Execute "update treport_tmp set header='" & fdate.value & "'"
...
Sub showdatainGrid()
    ...
    Dim tdr, tcr, tbal, top As Double
    ...
    ''date1_Last = DateAdd("d", Val(txtdays) * -1, fdate.value)
    ...
    For k2 = 1 To RS.RecordCount
        ...
        If RS(0) = "B3097 BOOK MAN & COMPANY (EM), VARANASI" Then
        '    MsgBox "a"
        End If
       ...
       If Check1_withoutCr.value = 1 Then
            ...
            If IIf((Val(((dr - cr) + op)) > 0), "Dr", "Cr") = "Dr" Then
             ...
             vs.TextMatrix(K, 2) = op
             vs.TextMatrix(K, 3) = dr
             vs.TextMatrix(K, 4) = cr
             vs.TextMatrix(K, 5) = (dr - cr) + op
...
'aaaaa:
    ...
    vs.FormatString = "SrNo|Name of Debitor|OP.Bal|DrAmt|CrAmt|Closing Bal."
...
Private Sub Commandshow_Click()
 ...
 'date1.Text = DateAdd("D", fdate, -120)

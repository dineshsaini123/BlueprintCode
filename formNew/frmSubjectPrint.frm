...
Begin VB.Form frmSubjectPrint 
   BackColor       =   &H00E0E0E0&
   Caption         =   "Subject Printing"
   ClientHeight    =   9192
   ClientLeft      =   60
   ClientTop       =   456
   ClientWidth     =   14052
   ...
   Begin VB.Frame panel 
      ...
      Begin VB.ComboBox txtFirmName 
         Height          =   315
         ItemData        =   "frmSubjectPrint.frx":0000
         Left            =   5490
         List            =   "frmSubjectPrint.frx":0002
      ...
      Begin VB.Frame buttonFrame 
         ...
         Begin VB.CommandButton cmdExit_12 
            ...
            Height          =   645
            Left            =   7470
            Picture         =   "frmSubjectPrint.frx":0004
            Style           =   1  'Graphical
            TabIndex        =   18
            Top             =   75
            Width           =   1230
         ...
         Begin VB.CommandButton cmdPrint_7 
            ...
            Height          =   645
            Left            =   6240
            Picture         =   "frmSubjectPrint.frx":0BE8
            Style           =   1  'Graphical
            TabIndex        =   17
            Top             =   75
            Width           =   1230
         ...
         Begin VB.CommandButton cmdUndo_5 
            ...
            Height          =   645
            Left            =   5010
            Picture         =   "frmSubjectPrint.frx":17CC
            Style           =   1  'Graphical
            TabIndex        =   16
            Top             =   75
            Width           =   1230
         ...
         Begin VB.CommandButton cmdEdit_4 
            ...
            Height          =   645
            Left            =   3780
            Picture         =   "frmSubjectPrint.frx":1D56
            Style           =   1  'Graphical
            TabIndex        =   15
            Top             =   75
            Width           =   1230
         ...
         Begin VB.CommandButton cmdDelete_3 
            ...
            Height          =   645
            Left            =   2535
            Picture         =   "frmSubjectPrint.frx":2198
            Style           =   1  'Graphical
            TabIndex        =   14
            Top             =   75
            Width           =   1230
         ...
         Begin VB.CommandButton cmdSave_2 
            ...
            Height          =   645
            Left            =   1305
            Picture         =   "frmSubjectPrint.frx":2D7C
            Style           =   1  'Graphical
            TabIndex        =   13
            Top             =   75
            Width           =   1230
         ...
         Begin VB.CommandButton cmdAdd_1 
            ...
            Height          =   645
            Left            =   75
            Picture         =   "frmSubjectPrint.frx":3960
            Style           =   1  'Graphical
            TabIndex        =   12
            Top             =   75
            Width           =   1230
   ...
   End
End
Attribute VB_Name = "frmSubjectPrint"
...
Private Sub cmdAdd_1_Click()
   ...
   'txtHeating.Text = MaxSNoNew("BillMaster", "bill_id", "Subject")
   txtHeating.text = MaxOrderNo(txtFirmName)
End Sub
Private Sub cmdDelete_3_Click()

If MsgBox("Want to delete ?", vbQuestion + vbYesNo) = vbYes Then
    con.Execute "delete from BillMaster where categories='Subject' and bill_id='" & txtHeating.text & "' and " & stringyear
    con.Execute "delete from Billtrans where categories='Subject' and bill_id='" & txtHeating.text & "' and " & stringyear
    Call cmdAdd_1_Click
End If
...
If rs1.State = 1 Then rs1.close
rs1.Open "select set_name,BookCode from Billtrans where categories='Subject' and bill_id='" & txtHeating.text & "' and " & stringyear & " order by set_name", con, adOpenKeyset, adLockReadOnly
While rs1.EOF = False
If Len(set1) > 0 Then
If (set1 = rs1(0)) Then
Else
...
If RS.State = 1 Then RS.close
RS.Open "select set_name from billtrans where categories='Subject' and BookCode= '" & rs1(1) & "' and set_name='" & rs1(0) & "'" & _
" and bill_id='" & txtHeating.text & "' and " & stringyear, con, adOpenKeyset, adLockReadOnly
If RS.EOF = False Then
...
con.Execute "update Billtrans set bookfont='" & fonttype & "',GPOrderPrinting=" & J & ",OrderPrinting=" & k1 & "" & _
" where categories='Subject' and set_name='" & rs1(0) & "' and bill_id='" & txtHeating.text & "' and BookCode= '" & rs1(1) & "' and " & stringyear
...
If MsgBox("Want To Print ?", vbInformation + vbYesNo) = vbYes Then
    ...
    CR.Reset
    CR.ReportFileName = rptPath & "/subjectPrint.rpt"
    CR.Connect = "filedsn=chitradsn;uid= " & sql_user & ";pwd=" & sql_pass
    CR.ReplaceSelectionFormula "{BillMaster.categories}='Subject' and {BillMaster.bill_id}='" & txtHeating.text & "'"
...
If RS.State = 1 Then RS.close
RS.Open "select * from BillMaster where categories='Subject' and bill_id='" & txtHeating.text & "' and " & stringyear, con, adOpenDynamic, adLockOptimistic
If RS.EOF = False Then
   con.Execute "delete from BillMaster where categories='Subject' and bill_id='" & txtHeating.text & "' and " & stringyear
   con.Execute "delete from billtrans where categories='Subject' and bill_id='" & txtHeating.text & "' and " & stringyear
End If
...
RS.Fields("categories").value = "Subject"
RS!setupid = setupid
RS!fyear = session
...
rs2.Open "select * from Billtrans where categories='Subject' and " & stringyear, con, adOpenDynamic, adLockOptimistic
...
rs2.Fields("categories").value = "Subject"
...
rs2.Fields("remdetails").value = vs.TextMatrix(I, 7)
...
rs1.MoveFirst
If rs1.EOF = False Then
paperdetails = rs1!SizeValue1 & " X " & rs1!SizeValue2 & " " & rs1!eco
End If
...
'If rs.State = 1 Then rs.close
RS.Open "select * from BillMaster where bill_id='" & txtHeating.text & "' and categories='Subject'", con, adOpenDynamic, adLockOptimistic
If RS.EOF = False Then
...
" where bm.fyear='" & session & "' and bm.setupid ='" & setupid & "' and b.bill_id='" & txtHeating.text & "' and bm.categories='Subject'", con, adOpenDynamic, adLockOptimistic
...
vs.TextMatrix(I, 7) = RS.Fields("remdetails").value & ""
...
Me.Caption = "Order For Subject Printing ..."
...
Me.Height = 9600
 ...
 s = ""
  Set RS = New ADODB.Recordset
 RS.Open "select Godwn as [Binder Name],Address from Godownmaster where " & stringyear & " and len(Godwn)>3", con, adOpenStatic, adLockReadOnly
...
'----------------------------------
 ...
 s = ""
 If RS.State = 1 Then RS.close
 RS.Open "select name from mastertbl  where Category='SubjectMaster' order by name", con
 While RS.EOF = False
 If s = "" Then
 s = RS(0)
 Else
...
vs.FormatString = "Books Name|Books Code|<To Print On|<Binder|>Qty|||Remarks"
vs.ColWidth(0) = 3500
vs.ColWidth(1) = 1200
vs.ColWidth(2) = 1500
vs.ColWidth(3) = 3500
...
If KeyCode = 113 Then
   value = "select distinct bill_id,dat as OrderDate,PrinterName from BillMaster where " & stringyear & " and categories='Subject' order by bill_id"
...
''    Next
''
''   If pname <> vs.TextMatrix(vs.RowSel, 2) Then
''      MsgBox "Select  " & pname & "  for this SET .. ", vbCritical
''      Exit Sub
''   End If

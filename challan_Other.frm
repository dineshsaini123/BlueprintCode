...
Begin VB.Form challan_Other 
   ...
   End
End
Attribute VB_Name = "challan_Other"
...
Dim i As Integer
InvoiceChallane.otherdiscount = 0
InvoiceChallane.otheramount = 0
    For i = 1 To mrow
        Grid1.row = i
        If rs.State = 1 Then
            rs.Close
        End If
        If InvoiceChallane.edit Then
                rs.Open "select * from cashctemp where " & stringyear, CON, adOpenKeyset, adLockReadOnly, adCmdText
        Else
        ...
        If Grid1.Text <> "" Then
            ...
            If Not rs.EOF Then
                If Trim(rs!DEBITORCREDIT) = Trim("Debit") Then
                    If rs!rate > 0 Then
                        Grid1.col = 1
                        InvoiceChallane.otherdiscount = InvoiceChallane.otherdiscount + ((InvoiceChallane.totalamount - InvoiceChallane.totaldiscount) * (Val(Grid1.Text) / 100))
                    Else
                        Grid1.col = 2
                        InvoiceChallane.otherdiscount = InvoiceChallane.otherdiscount + Val(Grid1.Text)
                    End If
                Else
                    If rs!rate > 0 Then
                        Grid1.col = 1
                        InvoiceChallane.otheramount = InvoiceChallane.otheramount + ((InvoiceChallane.totalamount - InvoiceChallane.totaldiscount) * (Val(Grid1.Text) / 100))
                    Else
                        Grid1.col = 2
                        InvoiceChallane.otheramount = InvoiceChallane.otheramount + Val(Grid1.Text)
                    End If
                End If
                End If
        End If
        rs.Close
    Next
    InvoiceChallane.mna.Caption = Format(myround(InvoiceChallane.totalamount + InvoiceChallane.otheramount - InvoiceChallane.totaldiscount - InvoiceChallane.otherdiscount, 2), "0.00")
    InvoiceChallane.mna.Caption = Format(myround(InvoiceChallane.totalamount + InvoiceChallane.otheramount - InvoiceChallane.totaldiscount - InvoiceChallane.otherdiscount, 2), "0.00")
    Me.Label3 = Format(InvoiceChallane.totalamount + InvoiceChallane.otheramount - InvoiceChallane.totaldiscount - InvoiceChallane.otherdiscount, "0.00")
    Me.labelbalance = Format(Val(Label3) + Val(Me.T1) + Val(Me.T2) - Val(Me.T3TEXT), "0.00")
      
    'Me.Label3 = Format(myround(InvoiceChallane.totalamount + InvoiceChallane.otheramount - InvoiceChallane.totaldiscount - InvoiceChallane.otherdiscount, 2), "0.00")
...
Sub otherabandon()
    ...
    Next
        ...
        challan_Other.T1 = ""
        challan_Other.T1TEXT = ""
        challan_Other.T2 = ""
        challan_Other.T2TEXT = ""
        challan_Other.T3TEXT = ""
End Sub
Private Sub amount_Change()
    If Grid1.row = lastrow And Grid1.col = lastcol Then
        ...
        If Grid1.col = 1 Then
            Grid1.col = 2
            Grid1.Text = Format(((InvoiceChallane.totalamount - InvoiceChallane.totaldiscount) * (Val(amount.Text) / 100#)), "0.00")
            Grid1.col = 1
        End If
    End If
...
Sub Commandreturn_Click()
    calc
    InvoiceChallane.labelbybank = T3TEXT
    challan_Other.Hide
...
calc
GROSS.Caption = InvoiceChallane.totalamount - InvoiceChallane.totaldiscount
...
Else
  Set rs = New ADODB.Recordset
  rs.Open "select * from CASHEND where  " & stringyear & " ORDER BY PRINTORDER", CON, adOpenKeyset, adLockReadOnly, adCmdText
End If
Grid1.col = 1
Grid1.row = 0
...
End If
rs.Close
rs.Open "select * from casha where Invoiceno=" + Trim(InvoiceChallane.I_NO.Text), CON, adOpenKeyset, adLockReadOnly, adCmdText
If Not rs.EOF Then
   challan_Other.T1 = Format(myround(rs!txt1a, 2), "0.00")
   challan_Other.T1TEXT = IIf(IsNull(rs!txt1), "", rs!txt1)
   challan_Other.T2 = Format(myround(rs!txt2a, 2), "0.00")
   challan_Other.T2TEXT = IIf(IsNull(rs!txt2), "", rs!txt2)
   challan_Other.T3TEXT = Format(myround(rs!baa, 2), "0.00")
End If
rs.Close

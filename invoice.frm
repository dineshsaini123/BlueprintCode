...
Begin VB.Form INVOICE 
   ...
   Begin VB.OptionButton optpay 
      ...
      Width           =   915
   End
   Begin VB.TextBox txtexpirynote 
      Height          =   285
      Left            =   1200
      TabIndex        =   70
      Text            =   "Two years after mfg. date"
      Top             =   5880
      Width           =   3045
   End
   ...
   Begin Crystal.CrystalReport cr1 
      ...
      _ExtentY        =   741
      _Version        =   348160
      ReportFileName  =   "F:\zenith_2009\REPORTS\invoicenew.rpt"
   ...
   Begin VB.ComboBox Bookcode 
      Height          =   2325
      ItemData        =   "invoice.frx":000C
      Left            =   2655
      List            =   "invoice.frx":000E
      Sorted          =   -1  'True
      Style           =   1  'Simple Combo
      TabIndex        =   43
      Top             =   2550
   ...
   Begin VB.Label Label20 
      AutoSize        =   -1  'True
      Caption         =   "INVOICE MANAGEMENT SYSTEM"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
...
Dim rs_rate As New ADODB.Recordset
...
rs1.Open "select * from invoicea where " & stringyear, CON, adOpenKeyset, adLockReadOnly, adCmdText
...
If Not rs1.EOF Then
    ...
    kk.Open "select * from invoiceb where  " & stringyear & "   and invoiceno=" + Trim(rs1!INVOICENO) + " order by discount,sno ", CON, adOpenKeyset, adLockReadOnly, adCmdText
    If Not kk.BOF Then
        kk.MoveFirst
        ...
        sno = 1
        Do While Not kk.EOF
            cdiscount = Val(kk!discount & "")
            Do While Val(kk!discount & "") = cdiscount
...
printagain2:
                ...
                Line = Line + 1
                tdata.Open "select sum(amount)as sumamt, sum(amount-netamount) as sumdis from invoiceb where  " & stringyear & "   and invoiceno=" + Trim(rs1!INVOICENO) + " and discount=" + Trim(str(cdiscount)) + " group by discount", CON, adOpenKeyset, adLockReadOnly, adCmdText
       ...
       If kk.State = 1 Then
             kk.Close
       End If
       kk.Open "Select * from invoicec where  " & stringyear & "   and invoiceno=" + Trim(Me.I_NO.Text), CON, adOpenKeyset, adLockReadOnly, adCmdText
       If Not kk.BOF Then
            Do While Not kk.EOF
                If kk!amount > 0 Then
        ...
        kk.Close
        Dim Va As Variant
        kk.Open "Select * from invoicea where  " & stringyear & "   and invoiceno=" + Trim(Me.I_NO.Text), CON, adOpenKeyset, adLockReadOnly, adCmdText
        If Not kk.BOF Then
             If kk!txt1a <> 0 Then
...
End If
If edit = False Then
  '  kk.Open "select max(invoiceno) from INVOICEA where   " & stringyear & "  ", con, adOpenStatic, adLockReadOnly, adCmdText
   ' If kk(0) > 0 Then
   '     kk.MoveLast
    ...
    End If
        ...
        For Each ctl In Me.Controls
            If TypeOf ctl Is MaskEdBox Or TypeOf ctl Is textbox Or TypeOf ctl Is ComboBox Then
                If UCase(Trim(ctl.Name)) <> UCase(Trim("I_NO")) And UCase(Trim(ctl.Name)) <> UCase(Trim("Genledger")) And UCase(Trim(ctl.Name)) <> UCase(Trim("I_DTOB")) And UCase(Trim(ctl.Name)) <> UCase(Trim("I_DT")) And UCase(Trim(ctl.Name)) <> UCase(Trim("bdated")) And UCase(Trim(ctl.Name)) <> UCase(Trim("txtexpirynote")) Then
                    ctl.Text = ""
                End If
                ctl.Enabled = False
            End If
                    ...
                    If Not rs.BOF Then
                        ...
                        'check1 = True
                         ...
                         '   If Not edit Then
                                ...
                                Grid1.col = 8
                                If Trim(Grid1.Text) = "" Then
                                    If rs_rate.State = 1 Then rs_rate.Close
                                    rs_rate.Open "select rate from INVOICEB where  " & stringyear & "   and  BOOKCODE='" & rs(0) & "' and SUBLEDGER='" & textbox.Text & "' order by INVOICENO", CON
                                    If rs_rate.EOF = False Then
                                    rs_rate.MoveLast
                                    Grid1.Text = Format(rs_rate!rate, "0.00")
                                    r = rs_rate!rate
                                    Else
...
If KeyAscii = 13 Then
  ...
  '********* vk
              ...
              '********
                ...
                If Trim(Grid1.Text) <> "" Then
                        Dim book() As String
                        book = Split(Grid1.Text, ":")
                        rs.Open "select * from books where " & stringyear & " and bookname='" & book(0) & "' and size1=" & Val(book(1)) & " and unit1='" & book(2) & "' and size2=" & Val(book(3)) & " and unit2='" & book(4) & "' and Quality='" & book(5) & "'", CON, adOpenKeyset, adLockReadOnly, adCmdText
                        If Not rs.BOF Then
                        If rs.EOF Then
...
'cr1.GroupSelectionFormula = ""
        cr1.ReportFileName = strrptpath & "\reports\invoicenew.rpt"
        cr1.Connect = constr
        cr1.SelectionFormula = "{invoicea.invoiceno} = " & INVOICE.I_NO.Text & " and {invoicea.fyear} = '" & main.session & "' and {invoicea.setupid} = " & main.setupid
...
inv1.Open "SELECT MAX(INVOICENO) FROM INVOICEA where " & stringyear, CON, adOpenKeyset, adLockReadOnly, adCmdText
    ...
    If edit = False Then
       If CON.Execute("SELECT MAX(INVOICENO) FROM INVOICEA where " & stringyear)(0) >= Val(Trim(Me.I_NO.Text)) Then
          ...
          Me.I_NO.Text = CON.Execute("SELECT MAX(INVOICENO) FROM INVOICEA where " & stringyear)(0) + 1
           inv = CON.Execute("SELECT MAX(INVOICENO) FROM INVOICEA where " & stringyear)(0)
...
Private Sub Commandedit_Click()
    ...
    DoEvents
    CON.Execute ("insert into invoicectmp  select * from invoicec where   " & stringyear & "   and INVOICENO = " + Trim(I_NO.Text))
    DoEvents
...
If s = 2 Then Exit Sub
frminvprintopt.txtinvoiceno.Text = I_NO.Text
frminvprintopt.Show 1
...
If addmode = False And edit = False And addoredit = False Then
MsgBox "Click Add to add new Invoice or Edit to modify existing Invoice"
Exit Sub
End If
   ...
   Exit Sub
   End If
   If station.Text = "" Then
   MsgBox "Enter Station Name"
   station.SetFocus
   Exit Sub
   End If
    ...
    If Trim(I_NO.Text) <> "" And Trim(i_dt.Text) <> "" And Trim(customercode.Text) <> "" Then
            ...
            If edit Then
                ...
                CON.Execute ("delete from invoiceb where   " & stringyear & "   and INVOICENO = " + Trim(I_NO.Text))
                CON.Execute ("delete from invoicec where   " & stringyear & "   and INVOICENO = " + Trim(I_NO.Text))
                CON.Execute "select * from invoicectmp where " & stringyear & " and INVOICENO = " + Trim(I_NO.Text)
            End If
            ...
            End If
            LAMOUNT = 0
            rs.Open "select * from invoicea where " & stringyear, CON, adOpenKeyset, adLockOptimistic, adCmdText
            If Not edit Then
again:
           If CON.Execute("SELECT MAX(INVOICENO) FROM INVOICEA where " & stringyear)(0) >= Val(Trim(Me.I_NO.Text)) Then
           ...
           End If
            ...
            rs!expirynote = txtexpirynote.Text
            ...
            End If
                ...
                For i = 1 To OTHERSALES.Grid1.Rows - 1
                        If i = 1 Then
                        rs!aexp1 = OTHERSALES.Grid1.TextMatrix(1, 0)
                        rs!aexp1rate = Val(OTHERSALES.Grid1.TextMatrix(1, 1))
                        rs!aexp1am = Val(OTHERSALES.Grid1.TextMatrix(1, 2))
                        End If
                        
                        If i = 2 Then
                        rs!aexp2 = OTHERSALES.Grid1.TextMatrix(2, 0)
                        rs!aexp2rate = Val(OTHERSALES.Grid1.TextMatrix(2, 1))
                        rs!aexp2am = Val(OTHERSALES.Grid1.TextMatrix(2, 2))
                        End If
                        
                        If i = 3 Then
                        rs!aexp3 = OTHERSALES.Grid1.TextMatrix(3, 0)
                        rs!aexp3rate = Val(OTHERSALES.Grid1.TextMatrix(3, 1))
                        rs!aexp3am = Val(OTHERSALES.Grid1.TextMatrix(3, 2))
                        End If
                        
                        If i = 4 Then
                        rs!lexp1 = OTHERSALES.Grid1.TextMatrix(4, 0)
                        rs!lexp1rate = Val(Val(OTHERSALES.Grid1.TextMatrix(4, 1)))
                        rs!lexp1am = Val(OTHERSALES.Grid1.TextMatrix(4, 2))
                        End If
                        
                        If i = 5 Then
                        rs!lexp2 = OTHERSALES.Grid1.TextMatrix(5, 0)
                        rs!lexp2rate = Val(OTHERSALES.Grid1.TextMatrix(5, 1))
                        rs!lexp2am = Val(OTHERSALES.Grid1.TextMatrix(5, 2))
                        End If
                        
                        If i = 6 Then
                        rs!lexp3 = OTHERSALES.Grid1.TextMatrix(6, 0)
                        rs!lexp3rate = Val(OTHERSALES.Grid1.TextMatrix(6, 1))
                        rs!lexp3am = Val(OTHERSALES.Grid1.TextMatrix(6, 2))
                        End If
                        
                        'OTHERSALES.Grid1.row = OTHERSALES.Grid1.row + 1
                Next
...
err1:
           If Not edit Then
            ...
            rs.Open "select * from invoiceb where " & stringyear, CON, adOpenKeyset, adLockOptimistic, adCmdText
            'rs!invoicedate = Me.i_dt.Text
            ...
            For i = 1 To maxrow
                ...
                If Trim(Grid1.Text) <> "" Then
                    ...
                    '   If Val(Trim(Grid1.Text)) > 0 Then
                         ...
                         rs!netamount = myround(mna.Caption, 0)
                         'LAMOUNT = 0
                         rs!sno = Grid1.row
                         rs!createdby = main.username
                        rs!createdon = Now
            ...
            rs.Open "select * from invoicec where " & stringyear, CON, adOpenKeyset, adLockOptimistic, adCmdText
            '/******
                'Dim I, x As Integer
                ...
                For i = 1 To OTHERSALES.mrow
                    ...
                    If Trim(OTHERSALES.Grid1.Text) <> "" Then
                        ...
                        End If
                        If edit Then
                        temprs.Open "select * from invoicectmp where " & stringyear & " and invoiceno=" & Val(Me.I_NO.Text), CON, adOpenKeyset, adLockReadOnly, adCmdText
                        ...
                        temprs.Open "select * from invoiceend where " & stringyear, CON, adOpenKeyset, adLockReadOnly, adCmdText
                        If OTHERSALES.Grid1.Text <> "" Then
                                temprs.Find "TEXT='" + Trim(OTHERSALES.Grid1.Text) + "'"
                                rs!Genledger = Trim(temprs!Genledger)
                                rs!subledger = Trim(temprs!subledger)
                        ...
                        If Val(Trim(OTHERSALES.Grid1.Text)) > 0 Then
                            rs!amount = (Me.totalamount - Me.totaldiscount) * (Val(Trim(OTHERSALES.Grid1.Text)) / 100)
                        Else
            ...
            rs.Update
                ...
                End If
                rs!In = CON.Execute("SELECT MAX(INVOICENO) FROM INVOICEA where " & stringyear)(0)
...
Private Sub Commandsearch_Click()
    Unload OTHERSALES
    CON.Execute "select * from invoicectmp where " & stringyear & " and invoiceno=" & Val(Me.I_NO.Text)
    Me.Enabled = False
    Call searchscreen.tempr(11, "invoice")
End Sub

Private Sub customercode_Change()
...
rs1.Open "Select * from sledger where " & stringyear & " and gledger='SUNDRY DEBTORS' and subledger='" + Trim(customercode.Text) + "'", CON, adOpenKeyset, adLockReadOnly, adCmdText
If rs1.EOF = False And Not (addmode = False And edit = False And addoredit = False) Then
txtpartyadd = rs1!subledger & vbCrLf & rs1!address1 & vbCrLf & rs1!ADDRESS2 & vbCrLf & rs1!ADDRESS3 & ""
txtpartyadd.ZOrder
txtpartyadd.Visible = True
Else
txtpartyadd.Visible = False
End If
End Sub


Private Sub customercode_Click()
partyadd
End Sub
...
Private Sub Form_Load()
    If s1 = 1 Then
       Label20 = "INVOICE ENTRY"
    Else
       Label20 = "ISSUE ITEM ENTRY"
    End If
    ...
    Me.Commandprintnh.Enabled = True
    If rs.State = adStateOpen Then rs.Close
    rs.Open "select * from books where   " & stringyear & " and upper(GROUPCODE)=upper('Yes') order by BOOKNAME, SIZE1, UNIT1, SIZE2, UNIT2, QUALITY", CON, adOpenKeyset, adLockReadOnly, adCmdText
    If Not rs.BOF Then
        Do While Not rs.EOF
    ...
    Else
       ...
       kk.Open "SELECT MAX(INVOICENO) FROM INVOICEA where " & stringyear, CON, adOpenKeyset, adLockReadOnly, adCmdText
       If kk(0) <> "" Then
           Me.I_NO.Text = Trim(str(kk(0) + 1))
       Else
          Me.I_NO.Text = "1"
...
Sub bookNo()
txtbookno.Caption = Fix((Val(I_NO.Text) - 1) / 50) + 1
End Sub
...
SetButton Commandadd, Commandedit, Commandsave, Commanddelete
    ...
    Else
        If rs.State = 1 Then
           rs.Close
        End If
        ''rs.Open "select * from invoicea where  " & stringyear & "  ", con, adOpenKeyset, adLockReadOnly, adcmdtext
        rs.Open "Select * from  INVOICEA where  " & stringyear & "   and  INVOICENO = " + Trim(I_NO.Text) + "", CON, adOpenStatic, adLockReadOnly
       ...
       ' mna.Caption = rs!netamount
        ...
        Me.txtRemark.Text = IIf(IsNull(rs!Remark), "", rs!Remark)
        txtexpirynote.Text = rs!expirynote & ""
         rs.Close
...
'Commandedit.Enabled = True
       ...
       ' Unload OTHERSALES
        CON.Execute "select * from invoicectmp where " & stringyear & " and INVOICENO = " & Trim(I_NO.Text)
       ' OTHERSALES.Form_Load
       ' 'rs.Open "select * from invoiceb where  " & stringyear & "  ", con, adOpenKeyset, adLockReadOnly, adcmdtext
       rs.Open "Select * from INVOICEB where  " & stringyear & "   and  INVOICENO =" + Trim(I_NO.Text) + " order by sno", CON, adOpenStatic, adLockReadOnly
       '' rs.Find "INVOICENO='" + Trim(I_NO.Text) + "'"
       Grid1.TopRow = 2
        If Not rs.EOF Then
...
If KeyAscii = 13 Then
            ...
            Select Case Grid1.col
                ...
                Grid1.Text = tempmeb.Text
                    ...
                    rs.Open "select * from books where   " & stringyear & " and GROUPCODE='Yes'", CON
                    If Not rs.BOF Then
    ...
    If Not rs1.EOF Then
        Print #1, " To,"; Tab(T1 - 3); rs1!subledger; Tab(T5); "Invoice No. : "; Trim(rs1!INVOICENO); Tab(T8 + 5); "Dt. "; Tab(T8 + 12); rs1!InvoiceDate 'Chr(27) + Chr(15);
            ...
            If kk.State = 1 Then
                kk.Close
            End If
            kk.Open "select * from invoiceb where   " & stringyear & "   and invoiceno=" + Trim(rs1!INVOICENO) + " order by discount,printorder", CON, adOpenKeyset, adLockReadOnly, adCmdText
            If Not kk.BOF Then
                kk.MoveFirst
                Dim cdiscount As Double
                Dim sno As Integer
...
printagain2:
                        ...
                        tdata.Open "select sum(amount) from invoiceb where  " & stringyear & "   and  invoiceno=" + Trim(rs1!INVOICENO) + " and discount=" + Trim(str(cdiscount)) + " group by discount", CON, adOpenKeyset, adLockReadOnly, adCmdText
                        If Not tdata.BOF Then

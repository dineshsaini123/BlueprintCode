...
Begin VB.Form crtitem 
   ...
   Begin VB.ComboBox Bookcode 
      Height          =   2325
      ItemData        =   "CrtItem.frx":0000
      Left            =   2640
      List            =   "CrtItem.frx":0002
      Sorted          =   -1  'True
      Style           =   1  'Simple Combo
      TabIndex        =   13
      Top             =   2490
...
If Not rs1.EOF Then
    ...
    kk.Open "select * from creditb where  " & stringyear & " and invoiceno=" + Trim(rs1!INVOICENO) + " order by discount,sno ", CON, adOpenKeyset, adLockReadOnly, adCmdText
    If Not kk.BOF Then
        kk.MoveFirst
...
printagain2:
                ...
                Line = Line + 1
                tdata.Open "select sum(amount)as sumamt, sum(amount-netamount) as sumdis from creditb where  " & stringyear & " and invoiceno=" + Trim(rs1!INVOICENO) + " and discount=" + Trim(str(cdiscount)) + " group by discount", CON, adOpenKeyset, adLockReadOnly, adCmdText
       ...
       If kk.State = 1 Then
             kk.Close
       End If
       kk.Open "Select * from creditc where  " & stringyear & " and invoiceno=" + Trim(Me.I_NO.Text), CON, adOpenKeyset, adLockReadOnly, adCmdText
       If Not kk.BOF Then
            Do While Not kk.EOF
                If kk!amount > 0 Then
        ...
        kk.Close
        Dim Va As Variant
        kk.Open "Select * from credita where  " & stringyear & " and invoiceno=" + Trim(Me.I_NO.Text), CON, adOpenKeyset, adLockReadOnly, adCmdText
        If Not kk.BOF Then
             If kk!txt1a <> 0 Then
...
inv1.Open "SELECT MAX(INVOICENO) FROM creditA where " & stringyear & "", CON, adOpenKeyset, adLockReadOnly, adCmdText
    ...
    If edit = False Then
       If CON.Execute("SELECT MAX(INVOICENO) FROM creditA where " & stringyear & "")(0) >= Val(Trim(Me.I_NO.Text)) Then
          ...
          Me.I_NO.Text = CON.Execute("SELECT MAX(INVOICENO) FROM creditA where " & stringyear & "")(0) + 1
           inv = CON.Execute("SELECT MAX(INVOICENO) FROM creditA where " & stringyear & "")(0)
...
Private Sub Commandedit_Click()
    ...
    CON.Execute ("DELETE from creditctmp where  " & stringyear & "")
    DoEvents
    CON.Execute ("insert into creditctmp  select * from creditc where  " & stringyear & " and INVOICENO = " + Trim(I_NO.Text))
    DoEvents
...
Private Sub Commandsave_Click()
    ...
    If Trim(I_NO.Text) <> "" And Trim(i_dt.Text) <> "" And Trim(customercode.Text) <> "" Then
            ...
            If edit Then
                ...
                CON.Execute ("DELETE from creditb where  " & stringyear & " and INVOICENO = " + Trim(I_NO.Text))
                CON.Execute ("DELETE from creditc where  " & stringyear & " and INVOICENO = " + Trim(I_NO.Text))
                CON.Execute "select * from creditctmp where " & stringyear
            End If
 ...
 '           rs!ORDERBY = Trim(Me.I_OB.Text)
  ...
  '          End If
           ...
           End If
            ...
            If Val(Trim(OTHERCredit.T3TEXT.Text)) <> 0 Then
...
'               rs!advicestatus = Me.txtadst.Text & ""
            ...
            End If
                ...
                For i = 1 To OTHERCredit.Grid1.Rows - 1
                        If i = 1 Then
'                        rs!aexp1 = OTHERCredit.Grid1.TextMatrix(1, 0)
  '                      rs!aexp1rate = Val(OTHERCredit.Grid1.TextMatrix(1, 1))
 '                       rs!aexp1am = Val(OTHERCredit.Grid1.TextMatrix(1, 2))
                        End If
                        
                        If i = 2 Then
   '                     rs!aexp2 = OTHERCredit.Grid1.TextMatrix(2, 0)
    '                    rs!aexp2rate = Val(OTHERCredit.Grid1.TextMatrix(2, 1))
     '                   rs!aexp2am = Val(OTHERCredit.Grid1.TextMatrix(2, 2))
                        End If
                        
                        If i = 3 Then
      '                  rs!aexp3 = OTHERCredit.Grid1.TextMatrix(3, 0)
       '                 rs!aexp3rate = Val(OTHERCredit.Grid1.TextMatrix(3, 1))
         '               rs!aexp3am = Val(OTHERCredit.Grid1.TextMatrix(3, 2))
                        End If
                        
                        If i = 4 Then
          '              rs!lexp1 = OTHERCredit.Grid1.TextMatrix(4, 0)
             '           rs!lexp1rate = Val(Val(OTHERCredit.Grid1.TextMatrix(4, 1)))
           '             rs!lexp1am = Val(OTHERCredit.Grid1.TextMatrix(4, 2))
                        End If
                        
                        If i = 5 Then
            '            rs!lexp2 = OTHERCredit.Grid1.TextMatrix(5, 0)
            '            rs!lexp2rate = Val(OTHERCredit.Grid1.TextMatrix(5, 1))
             '           rs!lexp2am = Val(OTHERCredit.Grid1.TextMatrix(5, 2))
                        End If
                        
                        If i = 6 Then
              '          rs!lexp3 = OTHERCredit.Grid1.TextMatrix(6, 0)
              '          rs!lexp3rate = Val(OTHERCredit.Grid1.TextMatrix(6, 1))
              '          rs!lexp3am = Val(OTHERCredit.Grid1.TextMatrix(6, 2))
                        End If
                        
                        'othercredit.Grid1.row = othercredit.Grid1.row + 1
                Next
...
err1:
           If Not edit Then
            ...
            rs.Update
                ...
                rs.Open "select * from tempcritnote where " & stringyear, CON, adOpenDynamic, adLockOptimistic, adCmdText
                If rs.BOF Then
                    rs.addNew
                rs!createdby = main.username
            rs!createdon = Now
            ...
            End If
                rs!In = CON.Execute("SELECT MAX(INVOICENO) FROM creditA where " & stringyear & "")(0)
...
Private Sub Form_Load()
    ...
    Grid1.ColWidth(5) = 1150
    Grid1.ColWidth(6) = 1150
...
Set rs = New ADODB.Recordset
    ...
    Else
        ...
        rs.Open "Select * from  creditA where " & stringyear & " and INVOICENO = " + Trim(I_NO.Text) + "", CON, adOpenStatic, adLockReadOnly
...
'Commandedit.Enabled = True
       ...
       rs.Open "Select * from creditB where " & stringyear & " and INVOICENO =" + Trim(I_NO.Text) + "", CON, adOpenStatic, adLockReadOnly
       '' rs.Find "INVOICENO='" + Trim(I_NO.Text) + "'"
       Grid1.TopRow = 2
        If Not rs.EOF Then
            ...
            Do While Not rs.EOF
               If Trim(rs!INVOICENO) = Trim(I_NO.Text) Then
                ...
                Grid1.col = 4
                Grid1.Text = IIf(IsNull(Trim(rs!btno)), "", Trim(rs!btno))
                Grid1.col = 5
                Grid1.Text = IIf(IsNull(Trim(rs!mgdate)), "", Trim(rs!mgdate))
                Grid1.col = 6
                Grid1.Text = IIf(IsNull(Trim(rs!expdate)), "", Trim(rs!expdate))
                Grid1.col = 7
    ...
    If Not rs1.EOF Then
        Print #1, " To,"; Tab(T1 - 3); rs1!subledger; Tab(T5); "Invoice No. : "; Trim(rs1!INVOICENO); Tab(T8 + 5); "Dt. "; Tab(T8 + 12); rs1!InvoiceDate 'Chr(27) + Chr(15);
            ...
            If kk.State = 1 Then
                kk.Close
            End If
            kk.Open "select * from creditb where  " & stringyear & " and invoiceno=" + Trim(rs1!INVOICENO) + " order by discount,printorder", CON, adOpenKeyset, adLockReadOnly, adCmdText
            If Not kk.BOF Then
                kk.MoveFirst
                Dim cdiscount As Double
                Dim sno As Integer
...
printagain2:
                        ...
                        tdata.Open "select sum(amount) from creditb where  " & stringyear & " and invoiceno=" + Trim(rs1!INVOICENO) + " and discount=" + Trim(str(cdiscount)) + " group by discount", CON, adOpenKeyset, adLockReadOnly, adCmdText
                        If Not tdata.BOF Then
